...
describe ShowQuery do
  let(:show_query_with_real_title)  { ShowQuery.new(title: real_title) }
  let(:show_query_with_fake_title)  { ShowQuery.new(title: fake_title) }
  ...
  describe '#query' do
    context 'with a title' do
      it 'returns an array of show results if there is a match' do
        VCR.use_cassette('real_title_show_query') do
          result = show_query_with_real_title.query
      ...
      it 'returns an empty array if there is no match' do
        VCR.use_cassette('fake_title_show_query') do
          result = show_query_with_fake_title.query
